...
class Live extends Component {
  ...
  componentDidMount() {
    Permissions.getAsync(Permissions.LOCATION)
      ...
      .catch(error => {
        console.log('Error getting Location perission: ', error);
    ...
    Location.watchPositionAsync(
      ...
      ({ coords }) => {
        ...
        if (newDirection != direction) {
          Animated.sequence([
            Animated.timing(bounceValue, { duration: 300, toValue: 1.04 }),
            Animated.spring(bounceValue, { duration: 300, friction: 4 })
          ]).start();
        }
  ...
  render() {
    ...
    if (status === 'undertermined') {
      return (
        <View style={styles.center}>
          <Foundation name="alert" size={50} />
          <Text>You need to enabel location services for this app.</Text>
          <TouchableOpacity onPress={this.askPermission}>
            <Text style={styles.buttonText}>Enable</Text>
          </TouchableOpacity>
        </View>
